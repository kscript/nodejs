...
module.exports = {
    index: function (id) {
        ...
        //key绑定core.media的key（前缀）
        that.assign('test', [{
            text: 123
        }, {
            text: 456
        }], 1);
        that.sql.Client.query("select * from " + that.core.db, function (err, data) {
    ...
    article: function (id) {
        ...
        try {
            ...
            that.sql.Client.query('update ' + that.core.db + " set view = view +1 where id =" + id);

            that.sql.Client.query("select * from " + this.core.db + " where id =" + id, function (err, data) {
                if (data) {
    ...
    tag: function (tag) {
        ...
        var sql = "select * from " + this.core.db + " where tag like '%" + tag + "%'";
    ...
    api: function (type) {
        ...
        if (/^POST$/i.test(TREE.method)) {
            ...
            that.sql.insert(obj, function (err, res) {
                res.time = that.core.fn.datatime(res, res.time);
